...
Given a string, `MSG`, of at most 63 characters, all lower case letters of the English alphabet, a symbol `SYM` (a single character), and a letter `FND` (a single character), write an assembly program that replaces all occurences of the letter `FND` in the string `MSG` with the symbol `SYM`, and stores the result in the `RES` string, also of at most 63 characters.
...
RES = a##embly
----------------------------------------
Replaced all occurences of 's' with '#'
```
...
What is the symbol?
%
What is the character to be replaced?
e
...
If you choose to implement your solution using file input and/or output, the format of the input file (`in.txt`) is the following (`MSG`, `SYM`, and `FND`, all on a separate line, in this order):
